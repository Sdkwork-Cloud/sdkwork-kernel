//! Installation lifecycle contract tests for OpenAI Codex.
//!
...
use sdkwork_agent_provider_codex::{codex_agent_installer, CODEX_CLI_PACKAGE, CODEX_CLI_VERSION};
...
fn installed_payload() -> String {
    npm_list_payload(&[(CODEX_CLI_PACKAGE, Some(CODEX_CLI_VERSION))])
}

#[test]
fn detects_missing_and_installed_states() {
    let root = TemporaryInstallRoot::new("codex-detect");
...
fn plans_with_policy_and_dry_runs_without_mutating() {
    let root = TemporaryInstallRoot::new("codex-plan");
...
fn installs_exact_packages_and_verifies_the_result() {
    let root = TemporaryInstallRoot::new("codex-install");
    ...
    assert!(install
        .args
        .iter()
        .any(|argument| argument == &format!("{CODEX_CLI_PACKAGE}@{CODEX_CLI_VERSION}")));
    assert!(install
        .args
        .iter()
...
fn upgrades_with_rollback_metadata_and_verifies() {
    let root = TemporaryInstallRoot::new("codex-upgrade");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
...
fn rollback_restores_the_snapshotted_dependency_state() {
    let root = TemporaryInstallRoot::new("codex-rollback");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(old_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(installed_payload()),
...
fn uninstall_removes_the_provider_and_verifies_absence() {
    let root = TemporaryInstallRoot::new("codex-uninstall");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(installed_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(npm_absent_payload()),
    ...
    assert!(uninstall
        .args
        .iter()
        .any(|argument| argument == CODEX_CLI_PACKAGE));
    assert!(
        installer
...
fn inventory_reports_only_proven_records() {
    let root = TemporaryInstallRoot::new("codex-inventory");
    let installed = installer(
        root.path(),
        ScriptedCommandExecutor::with_outputs(vec![ProcessAdapterCommandOutput::success(
            installed_payload(),
    ...
    assert_eq!(
        ...
        sdkwork_agent_kernel::AgentPackageSourceInfo::Registry {
            ...
            package_id: CODEX_CLI_PACKAGE.to_string(),
        }
    );
...
fn old_payload() -> String {
    npm_list_payload(&[(CODEX_CLI_PACKAGE, Some("0.0.1"))])
}
