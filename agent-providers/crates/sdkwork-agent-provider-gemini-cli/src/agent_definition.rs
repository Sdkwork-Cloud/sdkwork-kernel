...
pub fn gemini_cli_agent_manifest() -> AgentManifest {
    AgentManifest {
        ...
        description: "SDKWork-standard Gemini CLI SDK backed code-agent runtime.".to_string(),
...
pub fn gemini_cli_agent_definition() -> AgentDefinition {
    AgentDefinition::new(
        "definition.intelligence.gemini-cli",
        gemini_cli_agent_manifest(),
    )
    .with_provider_binding(
        AgentProviderBinding::new(
            "binding.gemini-cli.model",
    ...
    .with_model_selection(
        ModelSelectionPolicy::default_provider(ids::MODEL_PROVIDER_ID)
            .with_default_model_id("gemini-2.5-pro")
