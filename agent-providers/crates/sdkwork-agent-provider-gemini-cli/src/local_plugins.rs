...
impl LocalPluginProvider for GeminiCliLocalPluginProvider {
    ...
    fn discover(&self, request: &LocalPluginDiscoveryRequest) -> LocalPluginCatalog {
        ...
        for root in &request.roots {
            ...
            let skills = fs::read_dir(&skill_root)
                .ok()
                .into_iter()
                .flatten()
                .flatten()
                .filter_map(|entry| {
                    ...
                    path.is_file().then(|| LocalPluginSkillDescriptor {
                        skill_id: format!("skill.gemini.{skill_name}"),
                        name: skill_name,
                        description: None,
                        path,
                    })
            ...
            catalog.plugins.push(LocalPluginDescriptor {
                plugin_id: format!("plugin.intelligence.gemini-cli.{name}"),
                name: name.to_string(),
                version: value
                    .get("version")
