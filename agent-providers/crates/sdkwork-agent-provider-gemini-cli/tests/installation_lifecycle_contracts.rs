//! Installation lifecycle contract tests for Gemini CLI.
//!
...
use sdkwork_agent_provider_gemini_cli::{
    gemini_cli_agent_installer, GEMINI_CLI_PACKAGE, GEMINI_CLI_VERSION,
};

const AGENT_ID: &str = "agent.gemini";
...
fn installed_payload() -> String {
    npm_list_payload(&[(GEMINI_CLI_PACKAGE, Some(GEMINI_CLI_VERSION))])
}

#[test]
fn detects_missing_and_installed_states() {
    let root = TemporaryInstallRoot::new("gemini-cli-detect");
...
fn plans_with_policy_and_dry_runs_without_mutating() {
    let root = TemporaryInstallRoot::new("gemini-cli-plan");
...
fn installs_exact_packages_and_verifies_the_result() {
    let root = TemporaryInstallRoot::new("gemini-cli-install");
    ...
    assert!(install
        .args
        .iter()
        .any(|argument| argument == &format!("{GEMINI_CLI_PACKAGE}@{GEMINI_CLI_VERSION}")));
    assert!(install
        .args
        .iter()
...
fn upgrades_with_rollback_metadata_and_verifies() {
    let root = TemporaryInstallRoot::new("gemini-cli-upgrade");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
...
fn rollback_restores_the_snapshotted_dependency_state() {
    let root = TemporaryInstallRoot::new("gemini-cli-rollback");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(old_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(installed_payload()),
...
fn uninstall_removes_the_provider_and_verifies_absence() {
    let root = TemporaryInstallRoot::new("gemini-cli-uninstall");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(installed_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(npm_absent_payload()),
    ...
    assert!(uninstall
        .args
        .iter()
        .any(|argument| argument == GEMINI_CLI_PACKAGE));
    assert!(
        installer
...
fn inventory_reports_only_proven_records() {
    let root = TemporaryInstallRoot::new("gemini-cli-inventory");
    let installed = installer(
        root.path(),
        ScriptedCommandExecutor::with_outputs(vec![ProcessAdapterCommandOutput::success(
            installed_payload(),
    ...
    assert_eq!(
        ...
        sdkwork_agent_kernel::AgentPackageSourceInfo::Registry {
            ...
            package_id: GEMINI_CLI_PACKAGE.to_string(),
        }
    );
...
fn old_payload() -> String {
    npm_list_payload(&[(GEMINI_CLI_PACKAGE, Some("0.0.1"))])
}
