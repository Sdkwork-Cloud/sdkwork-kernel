//! Installation lifecycle contract tests for Mimo Code.
//!
...
use sdkwork_agent_provider_mimo_code::{
    mimo_code_agent_installer, MIMO_CODE_SDK_PACKAGE, MIMO_CODE_SDK_VERSION,
};

const AGENT_ID: &str = "agent.mimo-code";
...
fn installed_payload() -> String {
    npm_list_payload(&[(MIMO_CODE_SDK_PACKAGE, Some(MIMO_CODE_SDK_VERSION))])
}

#[test]
fn detects_missing_and_installed_states() {
    let root = TemporaryInstallRoot::new("mimo-code-detect");
...
fn plans_with_policy_and_dry_runs_without_mutating() {
    let root = TemporaryInstallRoot::new("mimo-code-plan");
...
fn installs_exact_packages_and_verifies_the_result() {
    let root = TemporaryInstallRoot::new("mimo-code-install");
    ...
    assert!(install
        .args
        .iter()
        .any(|argument| argument == &format!("{MIMO_CODE_SDK_PACKAGE}@{MIMO_CODE_SDK_VERSION}")));
    assert!(install
        .args
        .iter()
...
fn upgrades_with_rollback_metadata_and_verifies() {
    let root = TemporaryInstallRoot::new("mimo-code-upgrade");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
...
fn rollback_restores_the_snapshotted_dependency_state() {
    let root = TemporaryInstallRoot::new("mimo-code-rollback");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(old_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(installed_payload()),
    ...
    assert!(restore.args.iter().any(|argument| argument.contains("@")));
}

#[test]
fn uninstall_removes_the_provider_and_verifies_absence() {
    let root = TemporaryInstallRoot::new("mimo-code-uninstall");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(installed_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(npm_absent_payload()),
    ...
    assert!(uninstall
        .args
        .iter()
        .any(|argument| argument == MIMO_CODE_SDK_PACKAGE));
    assert!(
        installer
...
fn inventory_reports_only_proven_records() {
    let root = TemporaryInstallRoot::new("mimo-code-inventory");
    let installed = installer(
        root.path(),
        ScriptedCommandExecutor::with_outputs(vec![ProcessAdapterCommandOutput::success(
            installed_payload(),
    ...
    assert_eq!(
        ...
        sdkwork_agent_kernel::AgentPackageSourceInfo::Registry {
            ...
            package_id: MIMO_CODE_SDK_PACKAGE.to_string(),
        }
    );
...
fn old_payload() -> String {
    npm_list_payload(&[(MIMO_CODE_SDK_PACKAGE, Some("0.0.1"))])
}
