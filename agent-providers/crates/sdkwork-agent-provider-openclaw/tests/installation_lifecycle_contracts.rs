//! Installation lifecycle contract tests for OpenClaw.
//!
...
const AGENT_ID: &str = "agent.openclaw";
...
fn installed_payload() -> String {
    npm_list_payload(&[
        (OPENCLAW_PACKAGE, Some(OPENCLAW_PACKAGE_VERSION)),
        (OPENAI_SDK_PACKAGE, Some(OPENAI_SDK_VERSION)),
    ])
}

#[test]
fn detects_missing_and_installed_states() {
    let root = TemporaryInstallRoot::new("openclaw-detect");
...
fn plans_with_policy_and_dry_runs_without_mutating() {
    let root = TemporaryInstallRoot::new("openclaw-plan");
...
fn installs_exact_packages_and_verifies_the_result() {
    let root = TemporaryInstallRoot::new("openclaw-install");
    ...
    assert!(install
        .args
        .iter()
        .any(|argument| argument == &format!("{OPENCLAW_PACKAGE}@{OPENCLAW_PACKAGE_VERSION}")));
    assert!(install
        .args
        .iter()
        .any(|argument| argument == &format!("{OPENAI_SDK_PACKAGE}@{OPENAI_SDK_VERSION}")));
    assert!(install
        .args
        .iter()
...
fn upgrades_with_rollback_metadata_and_verifies() {
    let root = TemporaryInstallRoot::new("openclaw-upgrade");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
...
fn rollback_restores_the_snapshotted_dependency_state() {
    let root = TemporaryInstallRoot::new("openclaw-rollback");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(old_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(installed_payload()),
    ...
    assert!(restore
        .args
        .iter()
        .any(|argument| argument == &format!("{OPENAI_SDK_PACKAGE}@0.0.1")));
}

#[test]
fn uninstall_removes_the_provider_and_verifies_absence() {
    let root = TemporaryInstallRoot::new("openclaw-uninstall");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(installed_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(npm_absent_payload()),
    ...
    assert!(uninstall
        .args
        .iter()
        .any(|argument| argument == OPENCLAW_PACKAGE));
    assert!(uninstall
        .args
        .iter()
        .any(|argument| argument == OPENAI_SDK_PACKAGE));
    assert!(
        installer
...
fn inventory_reports_only_proven_records() {
    let root = TemporaryInstallRoot::new("openclaw-inventory");
    let installed = installer(
        root.path(),
        ScriptedCommandExecutor::with_outputs(vec![ProcessAdapterCommandOutput::success(
            installed_payload(),
    ...
    assert_eq!(
        ...
        sdkwork_agent_kernel::AgentPackageSourceInfo::Registry {
            ...
            package_id: OPENCLAW_PACKAGE.to_string(),
        }
    );
...
fn old_payload() -> String {
    npm_list_payload(&[
        (OPENCLAW_PACKAGE, Some("0.0.1")),
        (OPENAI_SDK_PACKAGE, Some("0.0.1")),
    ])
}
