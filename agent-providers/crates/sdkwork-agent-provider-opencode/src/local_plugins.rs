        ...
        for root in &request.roots {
            ...
            let skills = entries
                ...
                .filter_map(|entry| {
                    ...
                    let description = fs::read_to_string(&path).ok().and_then(|raw| {
                        raw.lines()
                            .find(|line| !line.trim().is_empty())
                            .map(ToOwned::to_owned)
                    });
                    Some(LocalPluginSkillDescriptor {
                        skill_id: format!("skill.opencode.{name}"),
                        name,
                        description,
                        path,
                    })
            ...
            catalog.plugins.push(LocalPluginDescriptor {
                ...
                root_path: command_root.clone(),
                manifest_path: command_root,
                source: LocalPluginSource::Workspace,
