//! Installation lifecycle contract tests for OpenCode.
//!
...
use sdkwork_agent_provider_opencode::{
    opencode_agent_installer, OPENCODE_SDK_PACKAGE, OPENCODE_SDK_VERSION,
};

const AGENT_ID: &str = "agent.opencode";
...
fn installed_payload() -> String {
    npm_list_payload(&[(OPENCODE_SDK_PACKAGE, Some(OPENCODE_SDK_VERSION))])
}

#[test]
fn detects_missing_and_installed_states() {
    let root = TemporaryInstallRoot::new("opencode-detect");
...
fn plans_with_policy_and_dry_runs_without_mutating() {
    let root = TemporaryInstallRoot::new("opencode-plan");
...
fn installs_exact_packages_and_verifies_the_result() {
    let root = TemporaryInstallRoot::new("opencode-install");
    ...
    assert!(install
        .args
        .iter()
        .any(|argument| argument == &format!("{OPENCODE_SDK_PACKAGE}@{OPENCODE_SDK_VERSION}")));
    assert!(install
        .args
        .iter()
...
fn upgrades_with_rollback_metadata_and_verifies() {
    let root = TemporaryInstallRoot::new("opencode-upgrade");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
...
fn rollback_restores_the_snapshotted_dependency_state() {
    let root = TemporaryInstallRoot::new("opencode-rollback");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(old_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(installed_payload()),
...
fn uninstall_removes_the_provider_and_verifies_absence() {
    let root = TemporaryInstallRoot::new("opencode-uninstall");
    let executor = ScriptedCommandExecutor::with_outputs(vec![
        ProcessAdapterCommandOutput::success(installed_payload()),
        ProcessAdapterCommandOutput::success(""),
        ProcessAdapterCommandOutput::success(npm_absent_payload()),
    ...
    assert!(uninstall
        .args
        .iter()
        .any(|argument| argument == OPENCODE_SDK_PACKAGE));
    assert!(
        installer
...
fn inventory_reports_only_proven_records() {
    let root = TemporaryInstallRoot::new("opencode-inventory");
    let installed = installer(
        root.path(),
        ScriptedCommandExecutor::with_outputs(vec![ProcessAdapterCommandOutput::success(
            installed_payload(),
    ...
    assert_eq!(
        ...
        sdkwork_agent_kernel::AgentPackageSourceInfo::Registry {
            ...
            package_id: OPENCODE_SDK_PACKAGE.to_string(),
        }
    );
...
fn old_payload() -> String {
    npm_list_payload(&[(OPENCODE_SDK_PACKAGE, Some("0.0.1"))])
}
