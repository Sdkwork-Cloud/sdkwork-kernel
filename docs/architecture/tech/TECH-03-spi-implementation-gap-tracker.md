...
| G-21 | Cross-provider live session control parity | P1 | `AGENT_PROVIDER_INTEGRATION_SPEC.md` section 6.1.1 | Provider-neutral L0 extension, runtime registry, L1 operation allowlists, and OpenCode official-SDK `interrupt`/`compact`/`fork` lane are implemented; Codex resident app-server and Claude streaming-query lanes are not wired | Users cannot yet control all provider sessions through one commercial runtime surface | Add resident Codex and Claude adapters, context usage/settings/resource refresh actions, event evidence, cancellation stress, and staging live proof |
