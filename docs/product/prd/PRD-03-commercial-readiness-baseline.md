...
| Runtime DB SQLite/Postgres parity | **Green** | Store-level pagination, SQLite WAL PRAGMAs, Postgres migrations, runtime persistence contract tests, and `RuntimeSessionWrites::append_message_with_event` idempotency tests for duplicate `message_id` retries and cross-session conflicts |
