...
| Infrastructure probes | **Partially complete** | Framework `/healthz`, `/readyz`, and `/livez` are present and readiness checks persistence. Redis and required provider-runtime readiness are separate promotion gates until composed into readiness |
...
| Internal-api pagination envelope | **Implementation present; contract tests required** | Session/message/task lists are cursor-only: the first request omits `cursor`, continuations pass through `pageInfo.nextCursor`, signed resource-scoped keyset paths use strict `page_size` `1..=200`, and tamper, continuity, bounded-query, and SQLite/PostgreSQL parity tests are release evidence |
| Distributed rate-limit and quota fail-closed | **Implementation present; failure drills required** | Redis-backed enforcement, exactly-once quota reconciliation, cancellation/disconnect release, and Redis outage behavior must pass targeted tests and staging failure drills |
...
| Provider subprocess true streaming | **Implementation present; soak gate required** | Incremental NDJSON/SSE and capacity release paths require provider-specific cancellation, slow-consumer, disconnect, and long-running soak evidence |
