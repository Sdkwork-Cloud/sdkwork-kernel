# Rig Complete Plugin Integration Design
...
- `sdkwork-agent-kernel` and `sdkwork-code-kernel` must not depend on `external/` or any integration crate.
- `external/rig` remains a reference source tree unless an explicit feature-gated live backend is enabled in the Rig integration crate.
...
- Plugin assembly belongs in `sdkwork-agent-integrations`, not in kernel core.
...
`SdkworkAgentIntegrationPlugin` is the stable assembly surface for SDKWork-owned external integrations.
...
Implement the complete Rig plugin path now. Keep other upstream integrations as mapping and manifest candidates until Rig proves the standard end to end.
