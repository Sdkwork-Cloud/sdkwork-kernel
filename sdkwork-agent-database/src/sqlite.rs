...
impl SqliteDatabase {
    ...
    pub fn new(path: &str) -> DatabaseResult<Self> {
        let conn = Connection::open(path).map_err(|e| {
            DatabaseError::Connection(format!("failed to open database: {}", e))
        })?;

        // Enable WAL mode for better concurrent access
        conn.execute_batch("PRAGMA journal_mode=WAL").map_err(|e| {
            DatabaseError::Connection(format!("failed to set WAL mode: {}", e))
        })?;
...
impl AgentDatabase for SqliteDatabase {
    fn execute(&self, sql: &str, params: &[&dyn DatabaseParam]) -> DatabaseResult<usize> {
        ...
        conn.execute(sql, rusqlite_params.as_slice()).map_err(|e| {
            DatabaseError::Query(format!("failed to execute: {}", e))
        })
    }
    ...
    fn query_many(&self, sql: &str, params: &[&dyn DatabaseParam]) -> DatabaseResult<Vec<Box<dyn DatabaseRow>>> {
        ...
        let mut stmt = conn.prepare(sql).map_err(|e| {
            DatabaseError::Query(format!("failed to prepare: {}", e))
        })?;
        ...
        let mut rows = stmt.query(rusqlite_params.as_slice()).map_err(|e| {
            DatabaseError::Query(format!("failed to query: {}", e))
        })?;

        let mut result = Vec::new();
        while let Some(row) = rows.next().map_err(|e| {
            DatabaseError::Query(format!("failed to fetch row: {}", e))
        })? {
            let mut values = HashMap::new();
            for (i, col_name) in column_names.iter().enumerate() {
                let value: rusqlite::types::Value = row.get(i).map_err(|e| {
                    DatabaseError::Query(format!("failed to get value: {}", e))
                })?;
    ...
    fn health(&self) -> DatabaseResult<bool> {
        ...
        let result: i32 = conn.query_row("SELECT 1", [], |row| row.get(0)).map_err(|e| {
            DatabaseError::Query(format!("health check failed: {}", e))
        })?;
...
impl DatabaseRow for SqliteRow {
    fn get_string(&self, column: &str) -> DatabaseResult<String> {
        let value = self.values.get(column).ok_or_else(|| {
            DatabaseError::Query(format!("column not found: {}", column))
        })?;
        ...
        match value {
            ...
            _ => Err(DatabaseError::Query(format!("column '{}' is not a string", column))),
        }
    }
    ...
    fn get_i64(&self, column: &str) -> DatabaseResult<i64> {
        ...
        match value {
            ...
            _ => Err(DatabaseError::Query(format!("column '{}' is not an integer", column))),
        }
    }
}
...
mod tests {
    ...
    fn sqlite_execute_and_query() {
        let db = SqliteDatabase::memory().expect("created");
        db.execute("CREATE TABLE test (id TEXT PRIMARY KEY, name TEXT)", &[]).expect("created");
        db.execute("INSERT INTO test (id, name) VALUES (?1, ?2)", &[&"1", &"test"]).expect("inserted");

        let rows = db.query_many("SELECT id, name FROM test WHERE id = ?1", &[&"1"]).expect("queried");
