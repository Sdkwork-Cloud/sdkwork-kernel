...
`POST .../model/stream` enforces the same per-tenant daily token quota as `POST .../model/invoke`. A supervisor owns the reservation until the blocking provider worker exits, reconciles successful streams to at least the conservative reservation (or the larger emitted-chunk estimate), reconciles partial failure/disconnect to emitted output, and only then publishes the terminal SSE event.

Single-resource JSON endpoints (manifest, health, diagnostics, snapshot, session create/read/close, task submit/read/cancel, permission decide, model invoke/cancel, tool execute) return `SdkWorkApiResponse` with `data.item`. Message send returns `201 Created` with `data.item` as `MessageTurnResponse`: required `userMessage`, optional `assistantMessage`, and `status: "completed"`. The user message, optional assistant message, `message.sent` events, `turn.completed`, and message count commit atomically before the response or event publication. `DELETE` session returns `204 No Content` with `X-SdkWork-Trace-Id`. Errors use `application/problem+json` (`ProblemDetail`) with numeric `code` and `traceId`.
...
- **Per-tenant daily token quotas:** optional JSON map `SDKWORK_TENANT_TOKEN_QUOTA_OVERRIDES` (`{"tenant-id":{"daily_tokens":N}}`) hard-limits model invoke and model stream when a tenant's UTC-day token consumption reaches the cap (`429 Too Many Requests`). Model invoke reconciles the reservation to provider-reported usage when present and otherwise keeps the conservative reservation. Model stream uses the supervised reconciliation described above. A `daily_tokens: 0` override blocks invocation and does not create adjustment usage. Redis-backed when `SDKWORK_RATE_LIMIT_REDIS_URL` / `SDKWORK_REDIS_URL` is configured.
- **Mutation idempotency:** declared JSON mutations require `Idempotency-Key` in production/non-loopback profiles. The server scopes keys by verified tenant, principal, method, route, query, and server-computed body SHA-256. Successful bounded JSON responses and bounded JSON 5xx responses are cached and replayed exactly; 4xx reservations are released. Non-JSON, oversized, or store-completion failures retain the reservation fail-closed because the business side effect may already have committed.
...
- **Runtime session persistence:** the server is an `authoritative-server` runtime and always uses the workspace PostgreSQL profile from `SDKWORK_DATABASE_*`. Startup rejects SQLite and retired application/module-prefixed database aliases. SQLite remains available only to in-memory or explicitly isolated client-local test fixtures.
...
- **Readiness dependencies:** `/readyz` fails closed on persistence health, migration version/index/foreign-key drift, and, in `cloud.production`, live rate-limit/idempotency Redis `PING` checks plus required typed provider health. Redis dependencies sharing a URL reuse one lazily initialized `ConnectionManager`; concurrent first probes coalesce connection creation instead of opening a connection per probe. The framework returns a generic 503 while detailed causes remain in server logs.
...
- `GET /metrics` exposes Prometheus text metrics:
  ...
  - `sdkwork_kernel_runtime_persistence_backend_info` (gauge `1`; server runtime label is `backend=postgres`, while `sqlite` is reserved for isolated test fixtures)
...
- Completed message persistence uses one `RuntimeSessionWrites::append_message_turn_with_events` transaction on authoritative PostgreSQL; the SQLite test adapter verifies equivalent client-local transaction semantics. Retried single-message appends remain idempotent; completed turns validate role order, session identity, unique message/event IDs, one `message.sent` per message, and exactly one `turn.completed` before any row is written.
- SSE handlers subscribe to the local bus before the bounded persistence replay, closing the replay/subscribe race. Live streams then combine local broadcast notifications with a one-second bounded persistent-store poll (`200` rows) so events committed by another pod or skipped by broadcast lag are recovered. The per-connection output channel is bounded to `64` items, and a bounded `1024`-ID deduplication window prevents poll/broadcast overlap from emitting the same persisted event twice.
...
- Response is SSE (`model.chunk` events followed by exactly one terminal `model.done` or `model.error`). Each chunk carries `modelRequestId`, `sequence`, and `content` per the OpenAPI schema; failure never emits a contradictory `model.done`.
