...
- Completed message persistence uses one `RuntimeSessionWrites::append_message_turn_with_events` transaction on SQLite and PostgreSQL. Retried single-message appends remain idempotent; completed turns validate role order, session identity, unique message/event IDs, one `message.sent` per message, and exactly one `turn.completed` before any row is written.
