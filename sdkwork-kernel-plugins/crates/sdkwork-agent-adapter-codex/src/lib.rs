...
impl ModelProvider for CodexModelProvider {
    ...
    fn invoke(&self, request: ModelRequest) -> KernelResult<ModelResponse> {
        ...
        Ok(ModelResponse::text(
            ...
            "provider.model.codex",
            format!("[Codex {}] Mock response to: {}", model_id, prompt),
        )
...
pub struct CodexLifecycleProvider {
    sessions: Mutex<HashMap<String, AgentSession>>,
}

impl CodexLifecycleProvider {
    pub fn new() -> Self {
        Self {
...
impl Default for CodexLifecycleProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl SessionLifecycleProvider for CodexLifecycleProvider {
    fn create_session(
        &self,
        agent_id: &str,
        user_ref: Option<&str>,
        ...
        let session_id = format!("codex.{}", uuid_simple());
...
mod tests {
    ...
    fn model_provider_invoke_mock() {
        ...
        assert_eq!(response.provider_id, "provider.model.codex");
