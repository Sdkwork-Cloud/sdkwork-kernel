...
impl MessageAdapter for OpenCodeMessageAdapter {
    ...
    fn to_agent_message(&self, external: &Self::ExternalMessage) -> KernelResult<AgentMessage> {
        ...
        if let Some(tool_calls) = &external.tool_calls {
            for tc in tool_calls {
                let mut part = AgentPart::tool_call_ref(
                    format!("opencode.tool_call.{}", tc.id),
                    &tc.id,
                );
        ...
        let mut message = AgentMessage::new(
            format!("opencode.msg.{}", uuid_simple()),
            role,
            parts,
        );
...
impl ModelProvider for OpenCodeModelProvider {
    ...
    fn invoke(&self, request: ModelRequest) -> KernelResult<ModelResponse> {
        ...
        Ok(ModelResponse::text(
            ...
            "provider.model.opencode",
            format!("[OpenCode {}] Mock response to: {}", model_id, prompt),
        )
...
pub struct OpenCodeLifecycleProvider {
    sessions: Mutex<HashMap<String, AgentSession>>,
}

impl OpenCodeLifecycleProvider {
    pub fn new() -> Self {
        Self {
...
impl Default for OpenCodeLifecycleProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl SessionLifecycleProvider for OpenCodeLifecycleProvider {
    fn create_session(
        &self,
        agent_id: &str,
        user_ref: Option<&str>,
        ...
        let session_id = format!("opencode.{}", uuid_simple());
...
mod tests {
    ...
    fn model_provider_invoke() {
        let provider = OpenCodeModelProvider::new();
        let request =
            ModelRequest::new("req.1", vec!["Explain async".to_string()])
    ...
    fn tool_provider_list_tools() {
        ...
        let search = tools.iter().find(|t| t.tool_id == "opencode.search").unwrap();
