...
pub fn rig_package_manifest() -> AgentPackageManifest {
    AgentPackageManifest::new(
        ids::AGENT_ID,
        "0.1.0",
        AgentPackageSource::registry("sdkwork", ids::AGENT_ID, "0.1.0"),
    )
    .with_lifecycle(AgentPackageLifecycle::installable())
    .expect("rig package lifecycle is valid")
    .with_provider_binding(AgentPackageProviderBinding::new(
        ids::INSTALLER_PROVIDER_ID,
        ids::CONFIGURATION_PROVIDER_ID,
    ))
    .expect("rig package provider binding is valid")
    ...
    .with_default_profile("profile.rig.local")
}
