...
fn rig_core_adapter_wraps_vector_search_without_leaking_rig_types() {
    let plan = sdkwork_agent_plugin_rig::RigCoreKnowledgeAdapter::vector_search_plan(
...
fn rig_planning_and_policy_provider_trait_manifests_match_rig_provider_ids() {
    ...
    let policy_provider = sdkwork_agent_plugin_rig::RigPolicyProvider::new();
...
fn rig_runtime_chat_rpc_enriches_model_requests_with_tool_and_memory_context() {
    ...
    .register_policy_provider(
        ...
        "0.1.0",
        sdkwork_agent_plugin_rig::RigPolicyProvider::new(),
    )
